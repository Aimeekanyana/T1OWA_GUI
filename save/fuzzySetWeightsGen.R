...
fuzzySetWeightsGen <- function(numObj, swOpt, lwOpt, specificWt, tri, tra, gau){	
  ...
  if(swOpt == 1) #"singletonMeet"
    ...
    #print(length(tri$i))
    #if(length(tri$i) > 0){
    #  for(i in length(LW):length(tri$i)){
    #    LW[[i]] <- tri$i[[i]]  
    #  }
    #}
    #
    #if(length(tra$i) > 0){
    #  for(i in length(LW):length(tra$i)){
    #    for(j in 1:length(tra$i)){
    #      LW[[i]] <- tra$i[[j]]
    #    }
    #  }
    #}
    #
    #if(length(gau$i) > 0){
    #  for(i in length(LW):length(gau$i)){
    #    for(j in 1:length(gau$i)){
    #      LW[[i]] <- gau$i[[j]]
    #    }
    #  }
    #}
	  ...
	  for(i in 1:length(LW)){
	    for(j in 1:length(LW[[i]])){
  	    for(k in 1:numObj)
  	    {
  	      ...
  	      #lines(seq(LW[[i]][[j]][[k]]$domain, LW[[i]][[j]][[k]]$domain, length=100), LW[[i]][[j]][[k]]$mfValue, lyt=2)
  	      lines(LW[[i]][[j]][[k]]$domain, LW[[i]][[j]][[k]]$mfValue, lyt=2)
  	      
  	      if(LW[[i]][[j]][[k]]$domain == 0)
  	        txtX = LW[[i]][[j]][[k]]$domain+0.02
  	      else if(LW[[i]][[j]][[k]]$domain == 1)
  	        txtX = LW[[i]][[j]][[k]]$domain-0.06
  	      else
  	        txtX = LW[[i]][[j]][[k]]$domain+0.02				
  	      
  	      text(txtX, 0.8, k, adj=c(0,0))
  	    }
	    }
	  }
	}
